...
class HoughLineDetector:
    def __init__(self, image_path, target_min_lines=5, target_max_lines=20, rho_res=1, theta_res=np.pi/180):
    ...
    def sobel_edge_detection(self):
        ...
        for i in range(1, height - 1):
            for j in range(1, width - 1):
                gx = np.sum(sobel_x * self.grayscale_image[i-1:i+2, j-1:j+2])
                gy = np.sum(sobel_y * self.grayscale_image[i-1:i+2, j-1:j+2])
